...
namespace Defsite {
	...
	public class Text : EntityOLD {
		...
		public Text(string text, int spacing = 10) {
			...
			AddComponent(new SpriteComponent(font.Texture, GeneratePositions(text), GenerateUVs(text), GenerateIndexBuffer(text.Length)));
		}
		...
		void SetText(string text) {
			...
			RemoveComponent(GetComponent<SpriteComponent>());
			AddComponent(new SpriteComponent(font.Texture, GeneratePositions(text), GenerateUVs(text), GenerateIndexBuffer(text.Length)));
		}
	}
}